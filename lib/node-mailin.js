...
NodeMailin.prototype.start = function(options, callback) {
        ...
        return retrieveRawEmail(connection)
            .then(function(rawEmail) {
                return Promise.all([
                    ...
                    getSpamReport(connection, rawEmail),
    ...
    function getSpamReport(connection, rawEmail) {
        if (configuration.disableSpamReport) {
            return Promise.resolve({error: true});
        }
        return mailUtilities.getSpamReportAsync(rawEmail).catch(function(err) {
            logger.error(
                connection.id + " Unable to check for spam."
            );
            logger.error(err);
            return {error: true};
        });
    ...
    function finalizeMessage(
        ...
        parsedEmail.spamReport = spamReport;
